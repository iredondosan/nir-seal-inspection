...
- **Hold-out global vigente**: `data/holdout_labels.csv` = 179 packs (**156 correctos / 23 defectuosos**). Es un **artefacto congelado**: `python data_prep/make_holdout.py` (por defecto) **carga y valida** ese fichero **sin re-muestrear**. El **protocolo** de generación —muestreo **solo de piezas NO revisadas** (sin fuga por construcción; el sellado se entrena solo con revisadas), **estratificado por producto** (prod1–prod5; prod6 solo entrenamiento), semilla propia `SEED=7`, fracciones **0,30 def / 0,20 good**— se ejecuta con `--regenerate`. Aviso: **re-muestrear sobre el snapshot de anotaciones actual da una composición ligeramente distinta** (el conjunto se depuró tras congelar el hold-out reportado), por lo que el hold-out **válido es el congelado**, no una regeneración. (La semilla del hold-out, 7, es distinta de la de los experimentos, 42.)
...
| **Tabla 4.5** · Transferencia ImageNet vs desde cero | tira: `python experiments/ablation_transfer.py`  ·  e2e: `python experiments/ablation_transfer_e2e.py` | `scratch_seal_1280.pt`, `defect_scratch_es.pt` vs desplegados | sellado Dice 0.967/0.949 · defecto tira 0.978/0.972 · **e2e 0.968/0.891** · anillo cerrado 179/153 · ES 69/141 ép. |
...
| **Tabla 4.7** · Ablación copy-paste (PI-4) | `python training/train_defect.py --sealjit --nopaste --out models/defect_nopaste.pt` · `python evaluation/eval_copypaste.py` | copy-paste ON vs OFF, resto fijo | ON 0.984/0.975 (23/23) · OFF 0.961/0.948 (19/23) → **ΔE2E +0.027** fuera del ruido; PI-4 respondida |
| **Tabla 4.8** · Ablación de aumentación | `python experiments/ablation_augment.py` | `defect_{strip.noaug,roll,jit,rolljit}.pt` sobre hold-out (23 def) | base 0.975/0.953 · +roll 0.980/0.983 · **+sealjit 0.984/0.975 (23/23, 3.8 % FP)** · +ambos 0.976/0.976 |
| **Tabla 4.9** · Despliegue (latencias CPU) | `python demo/bench_latency.py`  ·  INT8: `python deploy/quantize_int8.py` | ONNX (i7-12700K, 4 hilos) | sellado ONNX **342 ms** @1280 / **26 ms** @384 (torch 68 ms ref.); defecto 65/40 ms; pipeline E2E ~630 ms → ~95/min; INT8 **no desplegable** |
...
- El modelo `sealjit` de la ablación (Tabla 4.8, 0.975 e2e) y el `defect_strip.pt` desplegado (0.968

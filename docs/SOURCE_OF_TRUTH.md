...
- **tab:aug (4.8)** ✅ corregido (23 def, origen anclado 2026-07-12): baseline 0.975/0.953 · +roll 0.980/0.983 · **+sealjit 0.984/0.975** · +both 0.976/0.976. Nota: tras anclar la costura del desenrollado, *roll* ya no penaliza e2e (era artefacto); *sealjit* sigue siendo el mejor punto de operación (23/23 @ 3.8% FP).
- **tab:despliegue (4.9)** ✅ **RE-MEDIDO en CPU** (i7-12700K, 4 hilos, sin GPU) 2026-07-10. El 16.6/467/28× del LOG (junio) NO reproduce. Latencias actuales torch: Lite MobileNetV3-small (3.59 M) @384 **68 ms** / @512 130 / @1280 924; ResNet34-UNet (24.44 M) @384 **157 ms** / @512 285 → **~2.3× a igual resolución (384)**, 6.8× menos parámetros. ONNX Lite: @384 26 ms, @1280 **341 ms**, INT8@384 **19 ms**. Defecto resnet18 151 ms, TinyUNet 83 ms (tira 128×1536). Tabla y §4.10 actualizadas a estas cifras medidas.
- **tab:resumen (nueva, §4.10)** ✅ **agrega resultados existentes** (sin números nuevos): Dice 0.967 · LOPO 0.955±0.013 · AUROC E2E esperado 0.977±0.004 / desplegado 0.968 / mejor 0.983 · latencias ONNX de `results/latency.json` (sellado @1280 342 ms, defecto 65/40 ms, E2E ~630 ms→~95/min, opción rápida FP32@384 ~26 ms, AUROC E2E 0,969; INT8 no desplegable, ver `results/int8_quality.json`). Reconcilia esperado/desplegado/mejor E2E en un solo sitio; `tab:aug` reetiquetada `(config. desplegada)` con nota apuntando a `tab:umbral`.
...
**Semilla vs determinismo de GPU (por qué la MISMA receta da 21/23 y 23/23):** `training/train_defect.py:30` fija `random.seed(42)` + `np.random.seed(42)` + `torch.manual_seed(42)` — esto siembra el RNG (init de pesos, shuffle, split train/val y muestreo de la aumentación), reproducible entre corridas y compartido por las 4 variantes. **PERO no se activan los flags de determinismo de cuDNN** (`torch.use_deterministic_algorithms(True)`, `cudnn.deterministic=True`, `cudnn.benchmark=False`, `CUBLAS_WORKSPACE_CONFIG`). Por eso reentrenar la misma receta (mismo seed, mismo split) da pesos distintos: `defect_jit` (ablación) y `defect_strip.pt` (desplegado) —ambas sealjit— difieren por la selección de algoritmos de convolución y las sumas no atómicas del backward → **23/23 vs 21/23**. Verificado (2026-07-12) con el MISMO sellado y la MISMA tira para las dos cabezas: en `seal_1313`/`seal_2381` una cabeza puntúa ≈0,74 y la otra ≈0,000 — vuelco bimodal en 2-3 piezas de señal débil, no un roce con el umbral. El **AUROC** (agregado sobre el ranking) apenas se mueve (0.975 vs 0.968); el recuento a umbral fijo sí. Por eso se reporta el AUROC como métrica principal y no se fuerza el determinismo total (coste en velocidad; algunos kernels sin versión determinista). Coherente con §4.5 (“inestable entre reentrenamientos”) y con la nota de no-determinismo del 5-fold (sección 3).

**Ablación copy-paste (PI-4) — `evaluation/eval_copypaste.py`, `results/ablation_copypaste.json`, 2026-07-13 (tab:copypaste = Tabla 4.7):** aísla el copy-paste (ON vs OFF) con el resto fijo (ambos `--sealjit`, mismo seed/split, sellado @1280; flag nuevo `--nopaste` en `train_defect.py`). **ON** (`defect_jit`): GT 0.984 / E2E 0.975 / 23-23 / FP 6. **OFF** (`defect_nopaste`): GT 0.961 / E2E 0.948 / 19-23 / FP 4. ΔE2E **+0.027**, ΔGT **+0.023** → **FUERA del ruido** (±0.01), a diferencia de roll/sealjit: el copy-paste de defectos reales es **decisivo** con datos escasos y **responde PI-4** por ablación (no solo por diseño). Sin copy-paste el detector subdetecta (menos FP pero menos recall y peor AUROC, independiente del umbral).

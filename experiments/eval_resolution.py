#!/usr/bin/env python3
"""Boundary metrics (Dice, B-IoU, HD95, ASSD) for the seal at 384/512/1280 on prod2 + overall,
using the resolution-specific checkpoints. Completes tab:resolucion (only 1280 was saved).
Saves results/ablation_resolution.json."""
...
def agg(rows): return [float(np.mean([r[i] for r in rows])) for i in range(4)]
K=["dice","biou","hd95","assd"]; out={}
for res in [384,512,1280]:
    seal,sk=core.load_unet(f"{R}/models/best_lite_reviewed_{res}.pt",dev)
    rr={}
    for prod,p,m in val:
        ...
        rr.setdefault(prod,[]).append(metrics(m[y0:y1,x0:x1],pred))
    allm=[x for v in rr.values() for x in v]
    out[str(res)]={"input_res":sk["img"],"prod2":dict(zip(K,agg(rr["prod2"]))),"overall":dict(zip(K,agg(allm))),"n_overall":len(allm)}
    d=out[str(res)]["prod2"]; print("res %d: prod2 Dice %.3f B-IoU %.3f HD95 %.2f ASSD %.2f"%(res,d["dice"],d["biou"],d["hd95"],d["assd"]),flush=True)
save_results("ablation_resolution",out); print("saved results/ablation_resolution.json",flush=True)

...
from seal_inspection.paths import ROOT as R; PY = f"{R}/.venv/bin/python"; dev = "cuda" if torch.cuda.is_available() else "cpu"
...
for i in range(K):
    ...
    subprocess.run([PY, "training/train_defect.py", "--sealjit", "--kfold", str(K), "--fold", str(i), "--out", out],

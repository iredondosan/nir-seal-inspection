...
R = "/home/ubuntu/TFM/seal-inspection"; PY = f"{R}/.venv/bin/python"
...
for F, caps in FOLDS.items():
    ...
    open(hf, "w").write("\n".join(base + caps) + "\n")
    subprocess.run([PY, "src/make_strips.py", "--holdout", hf, "--out", f"data/strips_fold{F}"],
                   cwd=R, check=True, capture_output=True)
    subprocess.run([PY, "src/train_defect.py", "--strips", f"data/strips_fold{F}", "--sealjit",

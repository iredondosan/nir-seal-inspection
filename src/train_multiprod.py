...
OUT=f"{ROOT}/outputs/training_multiprod"; os.makedirs(OUT,exist_ok=True)
BASE=f"{ROOT}/models/best_lite.pt"
CKPT=f"{ROOT}/models/best_lite_multiprod.pt"; ONNX=f"{ROOT}/models/seal_lite_multiprod.onnx"
...
for xmlrel,imgrel,prod in DATASETS:
    ...
    for node in ET.parse(f"{ROOT}/{xmlrel}").getroot().findall('image'):
        nm=node.get('name'); p=f"{ROOT}/{imgrel}/{nm}"; m=seal_mask(node)
